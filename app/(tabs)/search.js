import {
    ...
    FlatList, SafeAreaView, StatusBar, Image, TextInput,
} from 'react-native';
...
export default function SearchScreen() {
    ...
    const filtered = ALL_PRODUCTS.filter(p => {
        ...
        const matchesCategory = activeCategory === 'Todos' || p.category === activeCategory.toLowerCase();
                    ...
                    <View style={styles.recentWrap}>
                        ...
                        <View style={styles.recentList}>
                            {RECENT_SEARCHES.map(term => (
                                <TouchableOpacity key={term} style={styles.recentChip} onPress={() => setSearchText(term)} activeOpacity={0.7}>
...
const styles = StyleSheet.create({
    ...
    searchBar: { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: CARD, borderRadius: 30, paddingHorizontal: 16, paddingVertical: 4, borderWidth: 1, borderColor: BORDER },
    searchBarFocused: { borderColor: PINK },
    searchInput: { flex: 1, color: WHITE, fontSize: 15, paddingVertical: 10 },
    chipsWrap: { marginBottom: 20 },
    chipsList: { paddingHorizontal: 20, gap: 10 },
    chip: { paddingHorizontal: 18, paddingVertical: 9, borderRadius: 20, borderWidth: 1, borderColor: BORDER, backgroundColor: CARD },
    ...
    recentChip: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: CARD, borderRadius: 20, paddingHorizontal: 14, paddingVertical: 8, borderWidth: 1, borderColor: BORDER },
    recentChipText: { fontSize: 13, color: WHITE, fontWeight: '500' },
    resultsHeader: { paddingHorizontal: 20, marginBottom: 14 },
    resultsCount: { fontSize: 13, color: MUTED },
    resultsList: { paddingHorizontal: 20, paddingBottom: 110 },
    separator: { height: 1, backgroundColor: BORDER, marginVertical: 4 },
    productRow: { flexDirection: 'row', alignItems: 'center', gap: 14, paddingVertical: 12 },
    rowImage: { width: 80, height: 80, borderRadius: 14, borderWidth: 1, borderColor: BORDER },
    rowInfo: { flex: 1 },
    ...
    discountBadge: { backgroundColor: '#2D1120', borderRadius: 10, paddingHorizontal: 7, paddingVertical: 2, borderWidth: 1, borderColor: '#4A1A30' },
    ...
    addBtn: { width: 32, height: 32, backgroundColor: PINK, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
